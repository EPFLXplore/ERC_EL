...
print("Timestamp | " + " | ".join([f"Cell {i} Voltage (V)" for i in cells]) + " | Balances | Temperature (C) | Current (A) | Voltage (V)")
...
def update():
    try:
        ...
        for i in cells:
            address = i + 1
            registers = read_registers(client, address, 4, slave_id=0xAA)
            voltages[i] = registers[0] / 10 if registers else None
        ...
        print(f"{sample_time} | " + " | ".join([f"{voltages[i]:.2f}" for i in cells]) +
              f" | {balances[:-2]} | {temperature:.1f} | {current:.2f} | {voltage:.2f}")
