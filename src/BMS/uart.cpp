...
void All_Events(SerialPort *serialPort, std::vector<uint8_t> command, uint16_t length){
	...
	printf("valeurs CRC read all events 0x%04X\n", crc_result_read_all_events);

	uint16_t CRC_MSB = crc_result_read_all_events >> 8; 
	uint16_t CRC_LSB = crc_result_read_all_events & 0xFF;

	
	command[2] = CRC_LSB;		//cf datasheet in all events section
	command[3] = CRC_MSB;
...
int main() {
	...
	// Create serial port object and open serial port at 115200 baud, 8 data bits, no parity bit(NONE), one stop bit (8n1),
	// and no flow control
